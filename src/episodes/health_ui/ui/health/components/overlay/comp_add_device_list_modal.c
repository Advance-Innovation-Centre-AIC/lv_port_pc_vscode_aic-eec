...
static void build_pairable_device_list(lv_obj_t *panel) {
  ...
  /* TODO BLE-SCAN-UI: Replace sim data below with real BLE scan cache.
   * 1. Before opening this modal, send IPC_CMD_BLE_SCAN_START to CM33:
   *      cm55_ipc_pipe_push_request(IPC_CMD_BLE_SCAN_START, NULL, 0);
   * 2. BLE scan results arrive in health_data_bridge.c → ble_scan_cache
   * 3. Replace health_ui_sim_data_get_device_count() with ble_scan_cache_get_count()
   * 4. Replace health_ui_sim_data_build_device_card() with ble_scan_cache_get(i)
   * 5. Map ipc_ble_scan_result_t fields to comp_device_card_data_t */
