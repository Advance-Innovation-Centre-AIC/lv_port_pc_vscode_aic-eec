 ...
 *              PC Simulator Mock Version
 *
 *              API-compatible with PSoC Edge E84 version.
 *              All hardware calls are replaced with simulated data.
 *
 * Components:
 *   - ADC (Analog-to-Digital Converter)
 *   - IMU (Inertial Measurement Unit - BMI270)
 *   - Temperature Sensor (internal)
 *   - CAPSENSE (I2C - not available on PC)
 *
 * Target: PC Simulator (MacOS/Windows/Linux)
 *
 * Usage:
 *   #include "sensors.h"
 *   aic_sensors_init();
...
/**
 * @brief Set simulated ADC value by percentage
 * @param channel ADC channel
...
/**
 * @brief Set simulated IMU data from struct
 * @param data Pointer to IMU data (float fields used)
 */
void aic_sensors_sim_set_imu(const aic_imu_data_t *data);

/**
 * @brief Update simulation tick (call periodically to animate sensors)
 *        Generates gentle sine-wave motion on accelerometer and gyroscope.
 */
