#include "comp_header_dashboard.h"

#include "../../layout/layout_fonts.h"
#include "../../layout/layout_tokens.h"

lv_obj_t *comp_header_dashboard_create(lv_obj_t *parent, const char *title,
                                       const char *subtitle) {
  lv_obj_t *panel;
  ...
  lv_label_set_text(title_label, (NULL != title) ? title : "Dashboard");
  lv_obj_set_style_text_color(title_label, lv_color_hex(HL_COLOR_TEXT_PRIMARY),
                              LV_PART_MAIN);
  lv_obj_set_style_text_font(title_label, HL_FONT_HEADING, LV_PART_MAIN);
  lv_obj_align(title_label, LV_ALIGN_TOP_LEFT, 0, 0);

  subtitle_label = lv_label_create(panel);
  lv_label_set_text(subtitle_label,
                    (NULL != subtitle) ? subtitle : "Layout-only preview");
